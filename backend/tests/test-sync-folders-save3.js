...
async function testUpdateAccount() {
  ...
  const userId = '2'; // правильный userId из БД
  ...
  const req = {
    ...
    body: {
      ...
      syncFolders: syncFolders // объект
    },
    get: function(header) {
  ...
  try {
    ...
    if (!responseSent) {
      console.log('⚠️ Контроллер не отправил ответ (возможно, ошибка)');
    }
    
    // Дадим немного времени на асинхронные операции
