...
router.get('/pl', async (req, res) => {
  try {
    ...
    for (const p of payments) {
      const key = p.categoryId || (p.kind === 'income' ? 'inc_other' : 'exp_other');
      const label = p.categoryName || (p.kind === 'income' ? 'Прочие поступления' : 'Прочие расходы');
    ...
    const byMonth = {};
    for (const p of payments) {
      const month = p.paymentDate ? String(p.paymentDate).substring(0, 7) : 'unknown';
      if (!byMonth[month]) byMonth[month] = { month, income: 0, expense: 0, profit: 0 };
      if (p.kind === 'income') byMonth[month].income += toNumber(p.amount);
      if (p.kind === 'expense') byMonth[month].expense += toNumber(p.amount);
      byMonth[month].profit = byMonth[month].income - byMonth[month].expense;
    }
