...
import { Badge, useTags, useCreateTag, useUpdateTag, useDeleteTag } from '@/components/ui/status-system';
...
export function TagEditor({ selectedModule, modules }: TagEditorProps) {
  const { t } = useTranslation();
  const { confirm } = useConfirm();
  const currentModule = modules.find(m => m.id === selectedModule);

  const { tags, isLoading } = useTags({ module: selectedModule });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-medium text-foreground">
          {t('settings.tags.for_module').replace('{module}', currentModule?.name || '')}
        </h3>
            ...
            <CardContent className="p-4">
              {editingId === tag.id ? (
                ...
                <div className="flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <Badge id={tag.id} type="tag" allowStyleEdit />
                    <div className="flex items-center gap-2">
                      {/* Индикаторы теперь внутри Badge */}
                    </div>
                    <span className="text-xs text-muted-foreground">ID: {tag.id}</span>
                  </div>
