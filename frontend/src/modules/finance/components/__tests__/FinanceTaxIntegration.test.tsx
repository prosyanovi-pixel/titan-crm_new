...
const FormWrapper = ({ children, defaultValues = {} }: { children: any, defaultValues?: any }) => {
    const form = useForm({
        defaultValues: {
            identifier: '',
            contractorId: 0,
...
describe('Finance TAX & VAT Integration', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should handle contractors without VAT correctly', async () => {
    const mockContractors = [
        { id: 2, name: 'USN IP', legalForm: 'IP' }
    ];

    (api.get as any).mockResolvedValue({
        taxRegime: { requiresNds: false },
        activeTaxes: []
    });
    ...
    // Select contractor
    await act(async () => {
        formRef.setValue('isTaxable', true); // Preset to true
        formRef.setValue('vatRate', 20);
        formRef.setValue('contractorId', 2);
    });

    // Check if VAT was reset to 0/false
    await waitFor(() => {
        expect(formRef.getValues('isTaxable')).toBe(false);
        expect(formRef.getValues('vatRate')).toBe(0);
    });
  });
});
